...
class MsfmValidator {
    ...
    public $modx = null;
    /**
     * @var Msfm $msfm A reference to the Msfm instance.
     * @access public
     */
    public $msfm = null;
     ...
     *
     * @param Msfm &$msfm A reference to the Msfm class instance.
     * @param array $config Optional. An array of configuration parameters.
     * @return MsfmValidator
     */
    function __construct(Msfm &$msfm,array $config = array()) {
        ...
        $this->config = array_merge(array(
            ...
            'customValidators' => !empty($this->msfm->config['customValidators']) ? explode(',',$this->msfm->config['customValidators']) : array(),
        ),$config);
    }
     ...
     *
     * @access public
     * @param MsfmDictionary $dictionary The fields to validate.
     * @param string $validationFields
     * @return array An array of field name => value pairs.
     */
    public function validateFields(MsfmDictionary $dictionary,$validationFields = '') {
     ...
     * either be a method name of MsfmValidator or a Snippet name.
    ...
    public function validate($key,$value,$type = '') {
        ...
        if (method_exists($this,$type) && !in_array($type,$invNames)) {
            ...
            if ($snippet) {
                ...
                $props = array_merge($this->msfm->config,array(
                    'key' => $key,
                    'value' => $value,
                    'param' => $param,
                    'type' => $type,
                ...
                /* no validator found */
                $this->modx->log(modX::LOG_LEVEL_ERROR,'[Msfm] Could not find validator "'.$type.'" for field "'.$key.'".');
                $validated = true;
            }
        } else {
            $this->modx->log(modX::LOG_LEVEL_INFO,'[Msfm] Validator "'.$type.'" for field "'.$key.'" was not specified in the customValidators property.');
            $validated = true;
        }
    ...
    public function addError($key,$value) {
        $errTpl = $this->modx->getOption('errTpl',$this->msfm->config,'<span class="error">[[+error]]</span>');
    ...
    public function required($key,$value) {
        ...
        return $success ? true : $this->_getErrorMessage($key,'vTextRequired','validate.field_required',array(
            'field' => $key,
            'value' => $value,
        ));
    }
    ...
    public function blank($key,$value) {
        return empty($value) ? true : $this->_getErrorMessage($key,'vTextBlank','validate.field_not_empty',array(
            'field' => $key,
            'value' => $value,
        ));
    }
    ...
    public function password_confirm($key,$value,$param = 'password_confirm') {
        if (empty($value)) return $this->modx->lexicon('validate.password_not_confirmed');
        $confirm = !empty($this->fields[$param]) ? $this->fields[$param] : '';
        if ($confirm != $value) {
            return $this->_getErrorMessage($key,'vTextPasswordConfirm','validate.password_dont_match',array(
                'field' => $key,
                'password' => $value,
    ...
    public function email($key,$value) {
        ...
        if (!$condition) {
            return $this->_getErrorMessage($key,'vTextEmailInvalid','validate.email_invalid',array(
                'field' => $key,
                'value' => $value,
            ));
        }
        ...
        if (!$condition) {
            ...
            if (sizeof($domain_array) < 2) {
                return $this->_getErrorMessage($key,'vTextEmailInvalidDomain','validate.email_invalid_domain',array(
                    'field' => $key,
                    'value' => $value,
                ));
            }
    ...
    public function minLength($key,$value,$param = 0) {
        ...
        if ($v < $param) {
            return $this->_getErrorMessage($key,'vTextMinLength','validate.min_length',array(
                'length' => $param,
                'field' => $key,
                'value' => $value,
            ));
    ...
    public function maxLength($key,$value,$param = 999) {
        $v = $this->config['use_multibyte'] ? mb_strlen($value,$this->config['encoding']) : strlen($value);
        if ($v > $param) {
            return $this->_getErrorMessage($key,'vTextMaxLength','validate.max_length',array(
                'length' => $param,
                'field' => $key,
                'value' => $value,
            ));
    ...
    public function minValue($key,$value,$param = 0) {
        if ((float)$value < (float)$param) {
            return $this->_getErrorMessage($key,'vTextMinValue','validate.min_value',array(
                'field' => $key,
                'passedValue' => $value,
                'value' => $param,
            ));
    ...
    public function maxValue($key,$value,$param = 0) {
        if ((float)$value > (float)$param) {
            return $this->_getErrorMessage($key,'vTextMaxValue','validate.max_value',array(
                'field' => $key,
                'passedValue' => $value,
                'value' => $param,
            ));
    ...
    public function contains($key,$value,$expr = '') {
        if (!preg_match('/'.$expr.'/i',$value)) {
            return $this->_getErrorMessage($key,'vTextContains','validate.contains',array(
                'field' => $key,
                'passedValue' => $value,
                'value' => $expr,
            ));
    ...
    public function range($key,$value,$ranges = '0-1') {
        ...
        if (count($range) < 2) return $this->modx->lexicon('validate.range_invalid');

        if ($value < $range[0] || $value > $range[1]) {
            return $this->_getErrorMessage($key,'vTextRange','validate.range',array(
                'min' => $range[0],
                'max' => $range[1],
                'field' => $key,
                'value' => $value,
    ...
    public function isNumber($key,$value) {
        if (!is_numeric(trim($value))) {
            return $this->_getErrorMessage($key,'vTextIsNumber','validate.not_number',array(
                'field' => $key,
                'value' => $value,
            ));
        }
    ...
    public function isDate($key,$value,$format = '%m/%d/%Y') {
        ...
        if ($ts === false || empty($value)) {
            return $this->_getErrorMessage($key,'vTextIsDate','validate.not_date',array(
                'format' => $format,
                'field' => $key,
                'value' => $value,
            ));
    ...
    public function islowercase($key,$value) {
        ...
        return strcmp($v,$value) == 0 ? true : $this->_getErrorMessage($key,'vTextIsLowerCase','validate.not_lowercase',array(
            'field' => $key,
            'value' => $value,
        ));
    }
    ...
    public function isuppercase($key,$value) {
        ...
        return strcmp($v,$value) == 0 ? true : $this->_getErrorMessage($key,'vTextIsUpperCase','validate.not_lowercase',array(
            'field' => $key,
            'value' => $value,
        ));
    }
    ...
    public function regexp($key,$value,$expression) {
        ...
        return !empty($matches) && !empty($matches[0]) == true ? true : $this->_getErrorMessage($key,'vTextRegexp','validate.not_regexp',array(
            'field' => $key,
            'value' => $value,
            'regexp' => $expression,
        ));
    ...
    protected function _getErrorMessage($field,$parameter,$lexiconKey,array $properties = array()) {
        if (!empty($this->msfm->config[$field.'.'.$parameter])) {
            $message = $this->msfm->config[$field.'.'.$parameter];
            $this->modx->lexicon->set($lexiconKey,$message);
            $this->modx->lexicon($lexiconKey,$properties);
        } else if (!empty($this->msfm->config[$parameter])) {
            $message = $this->msfm->config[$parameter];
            $this->modx->lexicon->set($lexiconKey,$message);
            $this->modx->lexicon($lexiconKey,$properties);
        } else {
